...
wavelet_bin_1=stack_wavelet_foci(tsStack,th-1*delta);
wavelet_bin_2=stack_wavelet_foci(tsStack,th+0*delta);
wavelet_bin_3=stack_wavelet_foci(tsStack,th+1*delta);
...
mkdir(name)
    ...
    %close all              
                 ...
                 foci_m=zeros(N_Dparamter,size(tsStack,2));
                 ...
                 allCN = vertcat(frame(fin).object.cellID); %finding ids per cell 
                 ind = find(allCN == N);
...
%                                    AB=tsStack(k).data;
 ...
 %                                   AB=A;
                        ...
                        % 
                                          ...
                                          if sum(abs(diff(bbb)))>(Diego_parameter+(p-1)*delta_D)
...
%                           
                            ...
                            try
                                ...
                                [foci4, foci2, tb, tc, tau]=cell_cycle_main2(foci,foci_esp,n_f);
